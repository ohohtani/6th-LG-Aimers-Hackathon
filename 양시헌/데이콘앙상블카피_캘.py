...
# ✅ 모델 학습, 캘리브레이션 및 예측 함수
def train_and_predict_with_calibration(X_data, y_data, target_data, best_cat_params, best_xgb_params):
    cat_preds, xgb_preds = np.zeros(len(target_data)), np.zeros(len(target_data))

    for train_idx, valid_idx in skf.split(X_data, y_data):
        # 🔹 CatBoost 학습 및 캘리브레이션
        ...
        cat_calibrator = CalibratedClassifierCV(estimator=cat_model, method='sigmoid', cv='prefit')
        cat_calibrator.fit(X_data.iloc[valid_idx], y_data.iloc[valid_idx])
        cat_preds += cat_calibrator.predict_proba(target_data)[:, 1] / skf.n_splits
        ...
        # XGBoost는 CalibratedClassifierCV와 직접 호환되지 않으므로 LightGBM-like 방식 적용
        xgb_preds_fold = xgb_model.predict(dvalid)
        xgb_calibrator = CalibratedClassifierCV(estimator=xgb.XGBClassifier(**best_xgb_params), method='sigmoid', cv='prefit')
        xgb_calibrator.fit(X_data.iloc[valid_idx], y_data.iloc[valid_idx])
        xgb_preds += xgb_calibrator.predict_proba(target_data)[:, 1] / skf.n_splits

    return cat_preds, xgb_preds

# ✅ 각 lane_count별 모델 튜닝 및 예측
best_cat1, best_xgb1 = tune_model(X1, y1)
cat_pred1, xgb_pred1 = train_and_predict_with_calibration(X1, y1, target1, best_cat1, best_xgb1)

best_cat2, best_xgb2 = tune_model(X2, y2)
cat_pred2, xgb_pred2 = train_and_predict_with_calibration(X2, y2, target2, best_cat2, best_xgb2)

best_cat3, best_xgb3 = tune_model(X3, y3)
cat_pred3, xgb_pred3 = train_and_predict_with_calibration(X3, y3, target3, best_cat3, best_xgb3)
...
submission.to_csv("ensemble_submission_lane_count_with_calibration.csv", index=False)
print("✅ 최종 제출 파일이 'ensemble_submission_lane_count_with_calibration.csv'로 저장되었습니다.")
