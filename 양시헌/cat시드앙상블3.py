...
# 🔑 --- 파생 변수 생성 함수 (나이 효과 + 요청된 3개 피처 추가) ---
def add_derived_features(df):
    ...
    # 🔑 --- 요청된 추가 피처 3개 ---
    df["embryo_age_interaction"] = df["이식된 배아 수"] * df["시술 당시 나이_수치"]       # 배아 수 × 나이 상호작용
    df["transfer_day_log"] = np.log1p(df["배아 이식 경과일"])                                # 이식 경과일 로그 변환
    df["fresh_embryo_product"] = df["수집된 신선 난자 수"] * df["총 생성 배아 수"]           # 신선 난자 × 총 배아 수
...
def objective(trial):
    ...
    for train_idx, valid_idx in kf.split(X, y):
        ...
        auc_scores.append(roc_auc_score(y_valid, model.predict_proba(X_valid)[:, 1]))
...
print(f"📁 새 최적화된 하이퍼파라미터 저장 완료: {new_params_save_path}")
print(f"🎯 최적 파라미터: {best_params}")

# 🔹 6. 저장된 파라미터 불러오기
with open(new_params_save_path, "rb") as f:
    best_params = pickle.load(f)
print(f"📁 불러온 파라미터: {best_params}")

# 🔹 7. 검증 세트 분리 및 random_seed 탐색
...
for seed in seed_range:
    ...
    val_scores.append((seed, roc_auc_score(y_val, val_preds)))
    print(f"Seed {seed} - ROC-AUC: {val_scores[-1][1]:.5f}")

# 🔑 상위 3개 시드로 앙상블
top_seeds = [s[0] for s in sorted(val_scores, key=lambda x: x[1], reverse=True)[:3]]
print(f"🚀 상위 3개 시드: {top_seeds}")
