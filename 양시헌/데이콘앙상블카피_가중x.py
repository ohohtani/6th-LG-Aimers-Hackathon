...
# ✔️ 테스트 데이터 확률 평균
ensemble_pred_proba = (_y_pred_cb_proba + _y_pred_xgb_proba) / 2

df_submission = pd.DataFrame({"ID": test_ids, "probability": ensemble_pred_proba})
submission_file_path = "soft_voting_submission.csv"
df_submission.to_csv(submission_file_path, index=False)

print(f"✅ 확률 기반 제출 파일이 '{submission_file_path}'로 저장되었습니다.")
